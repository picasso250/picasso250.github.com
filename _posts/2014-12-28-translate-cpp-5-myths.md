...
注意, 这个接口丢失了信息. 我不是在排序普通数据. 我们是在排序 `double`, 但是 `qsort` 不知道这个, 所以, 我们必须提供比较`double`的方法, 也必须提供`double`数据大小. 当然, 编译器其实已经完全知道这些信息了. 然而, `qsort` 的底层接口使得编译器利用这些类型信息变得不可能. 必须显示声明简单信息也给错误打开了大门. 那两个整数参数就容易搞混位置, 要是真的搞混了位置, 编译器又不可能提醒你. `compare()`函数有没有遵循C的三路比较的传统?
